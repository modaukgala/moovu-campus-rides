...
export default function RequestStatusPage({ params }: { params: { id: string } }) {
  ...
  async function fetchStatus() {
    try {
      setErr(null);
      const res = await fetch(`/api/ride-requests/${params.id}`, { cache: "no-store" });
  ...
  return (
    <main className="container" style={{ paddingTop: 26, paddingBottom: 40 }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <Link href="/" className="btnSecondary">Home</Link>
      </div>

      <p style={{ opacity: 0.75, marginTop: 8 }}>
        Request ID: <span style={{ opacity: 0.95, fontWeight: 800 }}>{params.id}</span>
      </p>
      ...
      {err && (
        <div className="card" style={{ marginTop: 12, borderColor: "rgba(227,28,61,0.45)" }}>
          ...
          <button className="btnSecondary" style={{ marginTop: 12 }} onClick={fetchStatus}>
            Retry
          </button>
        </div>
      )}
        ...
        <>
          <div className="card" style={{ marginTop: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
              ...
              <button className="btnSecondary" onClick={fetchStatus}>
                Refresh
              </button>
            </div>
            ...
            {typeof request.fare_amount === "number" && (
              <InfoRow label="Fare" value={`R${request.fare_amount}`} />
            )}
          </div>
          ...
          {driver && (
            <div className="card" style={{ marginTop: 12 }}>
              ...
              <a className="btnPrimary" style={{ marginTop: 12, width: "fit-content" }} href={`tel:${driver.phone}`}>
                Call Driver
              </a>
            </div>
          )}
...
function statusPill(status: string) {
  ...
  if (status === "assigned") return <span style={{ ...base, borderColor: "rgba(245,179,1,0.45)", background: "rgba(245,179,1,0.14)" }}>ASSIGNED</span>;
  if (status === "completed") return <span style={{ ...base, borderColor: "rgba(34,197,94,0.45)", background: "rgba(34,197,94,0.14)" }}>COMPLETED</span>;
  if (status === "cancelled") return <span style={{ ...base, borderColor: "rgba(227,28,61,0.45)", background: "rgba(227,28,61,0.14)" }}>CANCELLED</span>;
  return <span style={{ ...base, borderColor: "rgba(0,58,112,0.55)", background: "rgba(0,58,112,0.18)" }}>NEW</span>;
}